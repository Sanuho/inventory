...
$rec=mysqli_query($koneksi,"SELECT * FROM incoming_d where rec_h='$id'");
while ($getrec=mysqli_fetch_array($rec)){
$itm=$getrec['itm_cd'];
$qty=$getrec['qty'];
$req=$getrec['rec_h'];
...
$delete=mysqli_query($koneksi,"DELETE from incoming_h where rec_h='$id'");
if ($delete) {
//$gg=mysqli_query($koneksi,"DELETE from grgi_history where header='$id'");
$query=mysqli_query($koneksi,"DELETE from incoming_d where rec_h='$id'");
if ($query) {
	
  echo "<script type=\"text/javascript\">
              alert(\"Data Deleted!!\");
              window.location = \"../index.php?page=receive\"
              </script>";
