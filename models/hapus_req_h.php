...
$reg=mysqli_query($koneksi,"SELECT * FROM request_d where req_h='$id'");
while ($getrec=mysqli_fetch_array($reg)){
$itm=$getrec['itm_cd'];
$qty=$getrec['qty'];
$req=$getrec['req_h'];
...
$delete=mysqli_query($koneksi,"DELETE from request_h where req_h='$id'");
if ($delete) {
// $gg=mysqli_query($koneksi,"DELETE from grgi_history where header='$id'");
$query=mysqli_query($koneksi,"DELETE from request_d where req_h='$id'");
if ($query) {
