...
$wh = @$_SESSION['wh'];
if (isset($_POST['add'])) {
    $cek = mysqli_query($koneksi, "SELECT max(req_h) from request_h");
    $lastrow = mysqli_fetch_array($cek);
    $last = $lastrow[0];
    $check = mysqli_query($koneksi, "SELECT * FROM request_d where req_h='$last'");
    ...
    if ($num_rows > 0) {
        ...
        $carikode = mysqli_query($koneksi, "SELECT max(req_h) from request_h");
        ...
        if ($datakode) {
            ...
            $kode = (int)$nilaikode;
            $kode = $kode + 1;
            $hasilkode = "RH" . $tglkode . str_pad($kode, 4, "0", STR_PAD_LEFT);
        } else {
            $hasilkode = "RH" . $tglkode . "0001";
        }
        $waktu = gmdate("H:i:s", time() + 60 * 60 * 7);
        $tanggal = date('Y-m-d');
        $user = @$_SESSION['nama'];
        $sql = mysqli_query($koneksi, "INSERT Into request_h (req_h,date,time,user,flag,id_dept) values('$hasilkode','$tanggal','$waktu','$user',0,'$wh')");
    } else {
        $hasilkode = $last;
    }
}
                                                                ...
                                                                <select class="chosen" data-placeholder="No Data" name="category">
                                                                    ...
                                                                    ?>
                                                                        <optgroup label='<?php echo $isigrup['name']; ?>'>
                                                                            ...
                                                                            ?>
                                                                                <option value="<?php echo $isicombo['item_cd']; ?>"><?php echo $isicombo['item_cd'] . ' - ' . $isicombo['item_nm']; ?></option>
                                                                            <?php
                                                                            }
                                                                            ?>
                                                                        </optgroup>
                    ...
                    <div class="data-table-list">
                        <div class="table-responsive">
                            <table id="data-table-basic" class="table table-striped">
                                ...
                                <tfoot>
                                    <td><button type="button" class="btn btn-success" data-toggle="modal" data-target="#myModalthree"><i class="fa fa-plus-square"></i> Add</button>
                                        &nbsp;
